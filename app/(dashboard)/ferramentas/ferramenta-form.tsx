...
interface FerramentaFormProps {
  ferramenta?: Ferramenta & { kit?: KitItem[] }
...
export function FerramentaForm({ ferramenta, categorias, mode }: FerramentaFormProps) {
  ...
  const [kit, setKit] = useState<KitItem[]>(ferramenta?.kit ?? [])
  ...
  const addKitItem = () => setKit([...kit, { quantidade: 1, componente: '' }])

  const removeKitItem = (index: number) => setKit(kit.filter((_, i) => i !== index))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let ferramentaId = ferramenta?.id
      ...
      // Salva kit — deleta antigo e reinsere
      if (ferramentaId) {
        await supabase.from('ferramenta_kit').delete().eq('ferramenta_id', ferramentaId)

        const kitValido = kit.filter(k => k.componente.trim())
        if (kitValido.length > 0) {
          const { error: kitError } = await supabase.from('ferramenta_kit').insert(
            kitValido.map(k => ({
          ...
          if (kitError) {
            console.error('Erro ao salvar kit:', kitError)
            toast.warning('Ferramenta salva, mas houve erro ao salvar o kit')
          }
        }
      }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{mode === 'create' ? 'Nova Ferramenta' : 'Editar Ferramenta'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="marca" name="marca" value={formData.marca ?? ''} onChange={handleChange} placeholder="Ex: Bosch, DeWalt" disabled={isLoading} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="modelo">Modelo</Label>
                <Input id="modelo" name="modelo" value={formData.modelo ?? ''} onChange={handleChange} placeholder="Modelo" disabled={isLoading} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="numero_serie">Número de Série</Label>
                <Input id="numero_serie" name="numero_serie" value={formData.numero_serie ?? ''} onChange={handleChange} placeholder="Número de série" disabled={isLoading} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="valor_unitario">Valor Unitário (R$)</Label>
                <Input id="valor_unitario" name="valor_unitario" type="number" step="0.01" min="0" value={formData.valor_unitario ?? ''} onChange={handleNumberChange} placeholder="0,00" disabled={isLoading} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="localizacao">Localização</Label>
                <Input id="localizacao" name="localizacao" value={formData.localizacao ?? ''} onChange={handleChange} placeholder="Ex: Almoxarifado A" disabled={isLoading} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="data_aquisicao">Data de Aquisição</Label>
                <Input id="data_aquisicao" name="data_aquisicao" type="date" value={formData.data_aquisicao ?? ''} onChange={handleChange} disabled={isLoading} />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              Componentes do Kit
              <Button type="button" variant="outline" size="sm" onClick={addKitItem} disabled={isLoading}>
          ...
          <CardContent>
            {kit.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                <p className="text-sm">Kit: N/A</p>
                <p className="text-xs mt-1">Clique em "Adicionar" para incluir componentes</p>
              </div>
            ) : (
              <div className="space-y-2">
                {kit.map((k, i) => (
      ...
      <div className="flex items-center justify-between mt-6 pt-6 border-t">
        ...
        <Button type="submit" disabled={isLoading}>
          {isLoading ? (
            ...
            <><Save className="mr-2 h-4 w-4" />{mode === 'create' ? 'Criar Ferramenta' : 'Salvar Alterações'}</>
          )}
        </Button>
      </div>
    </form>
